...
NGame = {
	...
	GAME_SPEED_SECONDS = { 6000.0, 0.29, 0.21, 0.10, 0.06 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
...
NDiplomacy = {
	...
	BASE_SEND_ATTACHE_CP_COST = 0,				-- Command Power sent attache usage cost
...
NCountry = {
	...
	REINFORCEMENT_EQUIPMENT_DELIVERY_SPEED = 1,		-- Modifier for army equipment reinforcement speed
	REINFORCEMENT_MANPOWER_DELIVERY_SPEED = 30.0,	-- Modifier for army manpower reinforcement delivery speed (travel time)
	...
	BASE_STABILITY_WAR_FACTOR = 0,				-- Default stability war factor
	...
	BASE_MOBILIZATION_SPEED = 1,					-- Base speed of manpower mobilization  #in 1/1000 of 1 %
	...
	STARTING_COMMAND_POWER = 50.0,					-- starting command power for every country
	BASE_MAX_COMMAND_POWER = 300.0,					-- base value for maximum command power
	...
	ATTACHE_XP_SHARE = 0,						-- Country received xp from attaches
	...
	BASE_FUEL_GAIN_PER_OIL = 3.4,						-- base amount of fuel gained hourly per excess oil
...
NProduction = {
	...
	MIN_POSSIBLE_TRAINING_MANPOWER = 20000000,	-- How many deployment lines minimum can be training
	MIN_FIELD_TO_TRAINING_MANPOWER_RATIO = 0.9,	-- Ratio which % of army in field can be trained
...
NPolitics = {
	...
	ARMY_LEADER_COST = 1,					-- command power cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
	NAVY_LEADER_COST = 1,					-- command power cost for recruiting new leaders, 'this value' * number_of_existing_leaders_of_type
...
NDeployment = {
	BASE_DEPLOYMENT_TRAINING = 6,		-- Base training done each day during deployment.
},
...
NMilitary = {
	...
	--0% piercing gives 45% damage (was 50%).
	--More piercing gives extra 1%, up to 95%. Getting above 95% requires full piercing.
	...
	ZERO_ORG_MOVEMENT_MODIFIER = -0.2,		-- speed impact at 0 org.
	INFRA_ORG_IMPACT = 0.15,				-- scale factor of infra on org regain.
	...
	LAND_COMBAT_ORG_DAMAGE_MODIFIER = 0.015,       -- global damage modifier
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 1,	 			-- % of equipment lost to strength ratio in combat, so some % is returned if below 1
	SUPPLY_USE_FACTOR_MOVING = 1.5,                -- Deprecated/Unused
	SUPPLY_USE_FACTOR_INACTIVE = 0.95,			   -- Deprecated/Unused
	SUPPLY_GRACE = 168,							   -- troops always carry 3 days of food and supply
	...
	ENCIRCLED_DISBAND_MANPOWER_FACTOR = 0.5,       -- percentage of manpower returned when an encircled unit is disbanded
	ORG_LOSS_FACTOR_ON_CONQUER = 0.05,              -- percentage of (max) org loss on takign enemy province
	...
	LAND_EQUIPMENT_BASE_COST = 1,					-- Cost in XP to upgrade a piece of equipment one level is base + ( total levels * ramp )
...
NAir = {
	AIR_WING_FLIGHT_SPEED_MULT = 3,					-- Global speed multiplier for airplanes (affects fe.transferring to another base)
	...
	AIR_WING_XP_LEVELS = { 50, 250, 475, 700}, 							--Experience needed to progress to the next level
	...
	AIR_WING_XP_TRAINING_MAX = 250.0, 									--Max average XP achieved with training.
	...
	BOMBING_TARGETING_RANDOM_FACTOR = 0.05,							-- % of picking the wrong target
...
NAI = {
	...
	MAX_AHEAD_RESEARCH_PENALTY = 0,             -- Max ahead of time penalty AI will ever consider (this also includes BASE_YEAR_AHEAD_PENALTY_FACTOR, so not the raw time)
...
NSupply = {
	...
	CAPITAL_SUPPLY_BASE = 8.0, -- base supply for capital
	CAPITAL_SUPPLY_CIVILIAN_FACTORIES = 0.9, -- supply from one civilian factory
	CAPITAL_SUPPLY_MILITARY_FACTORIES = 0.9, -- supply from one military factory
	CAPITAL_SUPPLY_DOCKYARDS = 0.9, --supply from one naval factory
	...
	NAVAL_BASE_INITIAL_SUPPLY_FLOW = 7.0,
	...
	NODE_FLOW_BONUS_PER_RAIL_LEVEL = 0.7,
	...
	FLOATING_HARBOR_INITIAL_SUPPLY_FLOW = 4,
	FLOATING_HARBOR_STARTING_PENALTY_PER_PROVINCE = 0.4,
	FLOATING_HARBOR_ADDED_PENALTY_PER_PROVINCE = 0.6,

	FLOATING_HARBOR_BASE_SUPPLY = 20.0, -- supply given by a floating harbor
	FLOATING_HARBOR_BASE_DURATION = 28, -- duration of a full hp floating harbor
	FLOATING_HARBOR_DURATION_RATIO_AT_MIN_HP = 0.1,  -- duration mult for a harbor that was reduced to 0 hp
	...
	SUPPLY_FLOW_DROP_REDUCTION_AT_MAX_INFRA = 0.60, -- max infrastructure level will reduce the supply flow drop off by this ratio
	SUPPLY_FLOW_PENALTY_CROSSING_RIVERS = 0.05, -- crossing rivers introduces additional penalty

	 -- node flow terrain falloff is scaled by logistics curve based on distance(d) (scalar / (1+e^(-k(d-midpoint))))
	SUPPLY_FLOW_DIST_LOGISTICS_FALLOFF_K = 1.0, -- How steep the curve is
	...
	SUPPLY_HUB_FULL_MOTORIZATION_BONUS = 3,
	...
	SUPPLY_HUB_MOTORIZATION_MARGINAL_EFFECT_DECAY = 1.0,
	...
	RAILWAY_FLOW_PER_LEVEL = 7.0, 	-- how much additional flow a railway level gives
	...
	SUPPLY_BASE_MULT = 0.4,							-- multiplier on supply base values
	...
	DAYS_TO_START_GIVING_SUPPLY_AFTER_MOVING_SUPPLY_CAPITAL = 4,  -- the country will start gaining supply after this many days moving its capital
	...
	ALERT_VERY_LOW_SUPPLY_LEVEL = 0.5,			   -- At which point we show up the low and very low supply level alert. Value is in % of supplies supported vs required.
...
NIndustrialOrganisation = {
	ASSIGN_DESIGN_TEAM_PP_COST_PER_DAY = 0,					-- Cost in Political Power daily generation when one MIO is assigned to a research slot. If 0, cost is entirely disabled.
